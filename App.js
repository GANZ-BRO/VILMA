  ...
  rows.forEach((row) => {
    ...
    row.forEach((key) => {
      if (key === 'submit') {
        ...
        btn.textContent = key;
        btn.tabIndex = -1; // Megakadályozza a fókuszálást tabbillentyűvel
        btn.onclick = () => {
          if (key === '←') {
...
function showQuestion(index) {
  ...
  div.className = "question-container"; // Hozzáadott osztály a görgetésvezérléshez
  div.innerHTML =
